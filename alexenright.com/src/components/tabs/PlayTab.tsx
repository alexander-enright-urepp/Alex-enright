...
// import { Button } from '@/components/ui/Button' // IAP - Commented out
...
// import { PremiumModal } from '@/components/games/PremiumModal' // IAP - Commented out
...
const games: Game[] = [
  ...
  { id: 'tic-tac-toe', name: 'Tic Tac Toe', description: 'Beat the AI', icon: '⭕', locked: false },
  { id: 'snake', name: 'Snake', description: 'Classic arcade', icon: '🐍', locked: false },
  { id: 'fortune-cookie', name: 'Fortune Cookie', description: 'Daily fortune', icon: '🥠', locked: false },
  { id: 'hangman', name: 'Hangman', description: 'Guess the word', icon: '🎭', locked: false },
]

// PREMIUM GAMES CONFIG - Commented out for future IAP implementation
// const PREMIUM_GAMES = ['tic-tac-toe', 'snake', 'fortune-cookie', 'hangman']

export function PlayTab() {
  const [activeGame, setActiveGame] = useState<GameType | null>(null)
  
  // IAP STATE - Commented out for future implementation
  ...
  const handleGameClick = (game: Game) => {
    // IAP CHECK - Commented out for future implementation
  ...
  // IAP UNLOCK HANDLER - Commented out for future implementation
  ...
  return (
    <div className="p-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        {games.map((game) => (
          <button
            ...
            className="relative p-4 rounded-xl border-2 border-gray-200 bg-white text-left transition-all active:scale-95 hover:border-accent hover:shadow-md"
          >
      ...
      {/* 
      IAP SECTION - Commented out for future implementation
      ...
      {activeView === 'jobs' && (
        <div className="mt-6 p-4 bg-gradient-to-r from-accent/10 to-accent/5 rounded-xl">
          ...
          <Button onClick={() => setShowPremiumModal(true)} className="w-full">
            Unlock Premium - $2.99
          </Button>
        </div>
      )}
      ...
      {showPremiumModal && (
        <PremiumModal onClose={() => setShowPremiumModal(false)} onUnlock={() => setHasPremium(true)} />
      )}
      */}
    </div>
  )
